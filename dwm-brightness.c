...
#define BRIGHT_DIFF (unsigned int) 100
#define MIN_VAL (unsigned int) 0

static int safe_open(char *file, unsigned int flags)
{
    int fd;
    fd = open(file, flags);
...
static void safe_write(int fd, int new_val)
{
    char buff[MAX_BYTES];
